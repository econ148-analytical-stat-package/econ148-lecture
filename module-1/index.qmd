...
Supposed you have a population of 100 students adn want to select a sample of 20.
...
# Create a simulated dataset representing the population with barangays as clusters
set.seed(123)  # For reproducibility

population <- tibble(
  barangay = rep(paste0("Barangay_", 1:10), each = 100),  # 10 barangays, 100 individuals each
  id = 1:1000,  # Unique ID for each individual
  age = sample(18:80, 1000, replace = TRUE),  # Random age for each individual
  income = sample(10000:50000, 1000, replace = TRUE)  # Random income for each individual
)
...
selected_barangays <- population |> 
  distinct(barangay) |>  # Get unique barangays
  sample_n(n_clusters) |>  # Randomly select 3 barangays
  pull(barangay)  # Get the names of the selected barangays
